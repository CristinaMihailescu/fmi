...
lungimePlus xi 0 = xi
lungimePlus xi l = [0] ++ lungimePlus xi (l - 1)
...
normalizeazaLungime (n1, n2) = if length n1 >= length n2 then (n1, lungimePlus n2 (length n1 - length n2))
                               else (lungimePlus n1 (length n2 - length n1), n2)
...
lteN [] [] = True
lteN (x1:n1) (x2:n2) = if x1 < x2 then True
                       else if x1 == x2 then lteN n1 n2
                            else False

-- 2b (0,25p).  Scrieti o functie care ia doua Numere ca argumente
-- si verifica daca primul Numar este mai mic sau egal cu al doilea
lte :: Numar -> Numar -> Bool
lte n1 n2 = let (n1', n2') = normalizeazaLungime (n1, n2)
            in lteN n1' n2'
...
propagaFold' xs = let xi = map (\c -> 10 ^ c) [1..length xs]
				  in 
...
plusLista [] [] = []
plusLista (i:cs1) (j:cs2) = [i + j] ++ (plusLista cs1 cs2)
